...
export function CourseExplorer() {
  ...
  return (
    <section id="clases" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="rounded-[2rem] border border-white/80 bg-white/75 p-4 shadow-soft backdrop-blur md:p-6">
        <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
          <div className="relative">
            ...
            <input
              ...
              className="h-14 w-full rounded-2xl border border-slate-200 bg-white pl-12 pr-4 text-base font-medium text-slate-800 outline-none transition-colors focus-visible:border-amber-400 focus-visible:ring-4 focus-visible:ring-amber-200/70"
            />
          </div>
          ...
          <div className="rounded-2xl bg-slate-950 p-4 text-white">
            ...
            <p aria-live="polite" className="mt-2 text-xs tabular-nums text-white/65">
        ...
        <div
          ...
          className="mt-5 flex gap-2 overflow-x-auto pb-2 [overscroll-behavior-x:contain]"
        >
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((lecture) => (
          <LectureCard key={lecture.slug} lecture={lecture} checked={Boolean(progress[lecture.slug])} onToggle={toggleProgress} />
        ))}
      </div>

      {filtered.length === 0 ? (
        <div className="mt-10 rounded-3xl border border-dashed border-slate-300 bg-white/70 p-10 text-center">
          <p className="text-lg font-bold text-slate-900">No he encontrado clases con ese filtro.</p>
          <p className="mt-2 text-pretty text-slate-600">
            Prueba con &ldquo;Bayes&rdquo;, &ldquo;normal&rdquo;, &ldquo;esperanza&rdquo;, &ldquo;Markov&rdquo; o elimina el filtro de bloque.
          </p>
        </div>
      ) : null}
    </section>
...
function LectureCard({
  ...
  return (
    <article className="group relative overflow-hidden rounded-[1.75rem] border border-white/80 bg-white p-5 shadow-sm transition-shadow duration-300 motion-safe:transition motion-safe:hover:-translate-y-1 hover:shadow-soft">
      <div aria-hidden="true" className="absolute right-0 top-0 h-28 w-28 rounded-bl-full bg-gradient-to-br from-amber-200/60 to-sky-200/50" />
      <div className="relative flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-xs font-bold uppercase tracking-[0.18em] text-slate-400">{lecture.block}</p>
        </div>
        <button
          type="button"
          onClick={() => onToggle(lecture.slug)}
          aria-pressed={checked}
          aria-label={checked ? `Marcar "${lecture.title}" como pendiente` : `Marcar "${lecture.title}" como estudiada`}
      ...
      <h3 className="relative mt-5 line-clamp-2 text-balance text-xl font-black tracking-tight text-slate-950">{lecture.title}</h3>
      <p className="relative mt-3 line-clamp-4 text-pretty text-sm leading-6 text-slate-600">{lecture.goal}</p>
      ...
      <div className="relative mt-5 flex items-center justify-between gap-3">
        ...
        >
          Estudiar clase <Icon name="arrow" className="h-4 w-4" aria-hidden="true" />
        </Link>
